...
impl SteadyStudyTokenContract {
    ...
    pub fn ft_report_study_commit(&mut self, receiver_id: AccountId, memo: Option<String>, urls: &Vec<String>) {
        ...
        for element in urls.iter() {
            require!(self.used_urls.get(&element.to_string()) == None, "Not allows same url commit.");
        }
...
mod tests {
    ...
    fn test_ft_report_study_commit() {
        ...
        testing_env!(context.build());
        let mut contract = SteadyStudyTokenContract::new_default_meta(accounts(0).into(), TOTAL_SUPPLY.into());
        testing_env!(context
